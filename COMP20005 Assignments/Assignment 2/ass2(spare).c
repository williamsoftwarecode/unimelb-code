...
read_input(source_t source[], coordinates_t location[], int *n_poles, 
    ...
    while((ch=getchar()) != EOF) {
        ...
        /* test if maximum P-tagged input have been exceeded */
        ...
        /* test if maximum L-tagged input have been exceeded */
...
stage_one(allpoles_t P) {
	...
	for(i=0; i<P.n_poles; i++) {
		tot_illuminance+=calculate_illuminance(P.source[i], origin); 
	}
	...
	printf("Illuminance at (%4.1f,%2.1f):  %2.2f lux\n", 
...
stage_three(allpoles_t P) {
    ...
    for(i=1; i<=LENGTH; i++) { 
        for(j=1; j<=LENGTH; j++) {
            ...
            allcoordinates.x = 1.0*i; 
            allcoordinates.y = 1.0*j;
...
stage_four(allpoles_t P) {
	...
	for(j=LENGTH_CARPARK; j>0; j--) {
	    ...
	    /* reached the end of 78 column carpark */
	    /* next row of carparks */
	    printf("\n"); 
	}
